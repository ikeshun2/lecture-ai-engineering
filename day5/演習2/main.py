...
class DataValidator:
    ...
    def validate_titanic_data(data):
        ...
        # Great Expectationsを使用したバリデーション（新API対応）
        try:
            ...
            results.append(validator.expect_column_values_to_be_in_set("Pclass", [1, 2, 3]))
            results.append(validator.expect_column_values_to_be_in_set("Sex", ["male", "female"]))
            results.append(validator.expect_column_values_to_be_between("Age", 0, 100))
            results.append(validator.expect_column_values_to_be_between("Fare", 0, 600))
            results.append(validator.expect_column_values_to_be_in_set("Embarked", ["C", "Q", "S", ""]))
...
def test_inference_speed_and_accuracy():
    ...
    assert inference_time < 1.0, "推論に1秒以上かかっています"
    assert accuracy > 0.7, "精度が70%未満です"
